...
def main():
    try:
        ...
        pos1_min_z = heightinfo[1]      #ziel1
        pos2_min_z = heightinfo[5]      #ziel2
        pos3_min_z = heightinfo[9]
        pos4_min_z = heightinfo[13]
        pos5_min_z = heightinfo[17]
        
        pos1_gesucht = heightinfo[2]    #origin1
        pos2_gesucht = heightinfo[6]    #origin2
        pos3_gesucht = heightinfo[10]
        pos4_gesucht = heightinfo[14]
        pos5_gesucht = heightinfo[18]                       #19.5

        ################################## Höhe, die zur Anpeilung des spezifischen Blocks addiert werden muss (hälfte der Blockhöhe)
        hoehe1 = heightinfo[3]           #höhe1
        hoehe2 = heightinfo[7]           #höhe2
        hoehe3 = heightinfo[11]
        hoehe4 = heightinfo[15]
        hoehe5 = heightinfo[19]
        #print(hoehe1)
        #print(hoehe2)
        #print(hoehe3)
        #print(hoehe4)
        ...
        py.go_to_scan(float(pos1_gesucht[0]), float(pos1_gesucht[1]), (float(pos1_gesucht[2]) + hoehe1 + 0.3)) # 0.3m ueber dem Objekt
        input("<ENTER> Anfahren der Greifposition des 1. Objekts 1.1.1")
        py.go_to_pose_goal(float(pos1_gesucht[0]), float(pos1_gesucht[1]), (float(pos1_gesucht[2]) + hoehe1 + TCP + 0.1)) # 0.1m ueber das Objekt, damit es nicht verschoben wird
        input("<ENTER> Absenken auf Objektoberfläche 1.1.2")
        py.go_to_pose_goal(float(pos1_gesucht[0]), float(pos1_gesucht[1]), (float(pos1_gesucht[2]) + hoehe1 + TCP))
        # attach
        input("<ENTER HEBEN 1.1.3")
        py.go_to_pose_goal(float(pos1_gesucht[0]), float(pos1_gesucht[1]), (float(pos1_gesucht[2]) + hoehe1 + TCP + 0.25))

        ######################## Bewegung zum Ziel
        input("<ENTER> Anfahren der 1. Zielposition 1.2")
        py.go_to_pose_goal(float(pos1_min_z[0]), float(pos1_min_z[1]), (float(pos1_min_z[2]) + hoehe1 + TCP + 0.25))
        input("<ENTER> Ablegen 1.2.1")
        py.go_to_pose_goal(float(pos1_min_z[0]), float(pos1_min_z[1]), (float(pos1_min_z[2]) + hoehe1 + TCP))
        ...
        py.go_to_scan(float(pos2_gesucht[0]), float(pos2_gesucht[1]), (float(pos2_gesucht[2]) + hoehe2 + 0.3))
        input("<ENTER> Anfahren des 2. Objekts 2.1.2")
        py.go_to_pose_goal(float(pos2_gesucht[0]), float(pos2_gesucht[1]), (float(pos2_gesucht[2]) + hoehe2 + TCP + 0.1))
        input("<ENTER> Absenken auf Objektoberfläche 2.1.3")
        py.go_to_pose_goal(float(pos2_gesucht[0]), float(pos2_gesucht[1]), (float(pos2_gesucht[2]) + hoehe2 + TCP))
        input("<ENTER> HEBEN 2.1.4")
        # attach
        py.go_to_pose_goal(float(pos2_gesucht[0]), float(pos2_gesucht[1]), (float(pos2_gesucht[2]) + hoehe2 + TCP + 0.25))

        ######################## Bewegung zum Ziel
        input("<ENTER> Anfahren der 2. Zielposition 2.2")
        py.go_to_pose_goal(float(pos2_min_z[0]), float(pos2_min_z[1]), (float(pos2_min_z[2]) + hoehe2 + TCP + 0.25))
        input("<ENTER> Ablegen 2.2.1")
        py.go_to_pose_goal(float(pos2_min_z[0]), float(pos2_min_z[1]), (float(pos2_min_z[2]) + hoehe2 + TCP))
        input("<ENTER> HEBEN 2.2.2")
        # detach
        py.go_to_pose_goal(float(pos2_min_z[0]), float(pos2_min_z[1]), (float(pos2_min_z[2]) + hoehe2 + TCP + 0.15))

        ######################## Bewegung zum Ursprung des 3. gesuchten Objekt
        input("<ENTER> Anfahren der Position für Scan 3.1")
        py.go_to_scan(float(pos3_gesucht[0]), float(pos3_gesucht[1]), (float(pos3_gesucht[2]) + hoehe3 + 0.3))
        input("<ENTER> Anfahren des 3. Objekts 3.1.2")
        py.go_to_pose_goal(float(pos3_gesucht[0]), float(pos3_gesucht[1]), (float(pos3_gesucht[2]) + hoehe3 + TCP + 0.1))
        input("<ENTER> Absenken auf Objektoberfläche 3.1.3")
        py.go_to_pose_goal(float(pos3_gesucht[0]), float(pos3_gesucht[1]), (float(pos3_gesucht[2]) + hoehe3 + TCP))
        input("<ENTER> HEBEN 3.1.4")
        # attach
        py.go_to_pose_goal(float(pos3_gesucht[0]), float(pos3_gesucht[1]), (float(pos3_gesucht[2]) + hoehe3 + TCP + 0.25))

        ######################## Bewegung zum Ziel
        input("<ENTER> Anfahren der 3. Zielposition 3.2")
        py.go_to_pose_goal(float(pos3_min_z[0]), float(pos3_min_z[1]), (float(pos3_min_z[2]) + hoehe3 + TCP + 0.15))
        input("<ENTER> Ablegen 3.2.1")
        py.go_to_pose_goal(float(pos3_min_z[0]), float(pos3_min_z[1]), (float(pos3_min_z[2]) + hoehe3 + TCP))
        ...
        py.go_to_scan(float(pos4_gesucht[0]), float(pos4_gesucht[1]), (float(pos4_gesucht[2]) + hoehe4 + 0.3))
        input("<ENTER> Anfahren des 4. Objekts 4.1.2")
        py.go_to_pose_goal(float(pos4_gesucht[0]), float(pos4_gesucht[1]), (float(pos4_gesucht[2]) + hoehe4 + TCP + 0.1))
        input("<ENTER> Absenken auf Objektoberfläche 4.1.3")
        py.go_to_pose_goal(float(pos4_gesucht[0]), float(pos4_gesucht[1]), (float(pos4_gesucht[2]) + hoehe4 + TCP))
        input("<ENTER> HEBEN 4.1.4")
        # attach
        py.go_to_pose_goal(float(pos4_gesucht[0]), float(pos4_gesucht[1]), (float(pos4_gesucht[2]) + hoehe4 + TCP + 0.25))

        ######################## Bewegung zum Ziel
        input("<ENTER> Anfahren der 4. Zielposition 4.2")
        py.go_to_pose_goal(float(pos4_min_z[0]), float(pos4_min_z[1]), (float(pos4_min_z[2]) + hoehe4 + TCP + 0.15))
        input("<ENTER> Ablegen 4.2.1")
        py.go_to_pose_goal(float(pos4_min_z[0]), float(pos4_min_z[1]), (float(pos4_min_z[2]) + hoehe4 + TCP))
        ...
        py.go_to_scan(float(pos5_gesucht[0]), float(pos5_gesucht[1]), (float(pos5_gesucht[2]) + hoehe5 + 0.3))
        input("<ENTER> Anfahren des 5. Objekts 5.1.2")
        py.go_to_pose_goal(float(pos5_gesucht[0]), float(pos5_gesucht[1]), (float(pos5_gesucht[2]) + hoehe5 + TCP + 0.10))
        input("<ENTER> Absenken auf Objektoberfläche 5.1.3")
        py.go_to_pose_goal(float(pos5_gesucht[0]), float(pos5_gesucht[1]), (float(pos5_gesucht[2]) + hoehe5 + TCP))
        input("<ENTER> HEBEN 5.1.4")
        # attach
        py.go_to_pose_goal(float(pos5_gesucht[0]), float(pos5_gesucht[1]), (float(pos5_gesucht[2]) + hoehe5 + TCP + 0.25))

        ######################## Bewegung zum Ziel
        input("<ENTER> Anfahren der 5. Zielposition 5.2")
        py.go_to_pose_goal(float(pos5_min_z[0]), float(pos5_min_z[1]), (float(pos5_min_z[2]) + hoehe5 + TCP + 0.15))
        input("<ENTER> Ablegen 5.2.1")
        py.go_to_pose_goal(float(pos5_min_z[0]), float(pos5_min_z[1]), (float(pos5_min_z[2]) + hoehe5 + TCP))
